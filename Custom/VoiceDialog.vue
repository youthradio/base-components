<template>
  <div ref="container">
    ...
    <div v-for="(dialog, i) in dialogs" :key="dialog.passage" class="dialog">
      ...
      >
        <div class="small-play">
          <a href="#" @click.prevent="togglePassage(i)">
          ...
          <a
            href="#"
            @click.prevent="
              tweetMessage(dialog.passage, guestsMap.get(dialog.speaker))
            "
          >
            <svg
  ...
  methods: {
    tweetMessage(msg, author) {
      ...
      const tweet = `#InTheBlackMirror: “${msg.replace(
        /(\r\n|\n|\r)/gm,
        ''
      )}” by ${author.name} @${author.twitter} - via @itsyrmedia ${url}`
